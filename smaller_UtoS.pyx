...
def t_loop(double duration, int[:] mt_region, int[:] positions, double[:] rates, int global_mode):
    cdef double T = 0
    cdef double t = 0
...
# #        
        ...
        # if the global recruitment-rate M-catalysed change of U to M (recruited conversion) is chosen
            ...
            if global_mode == 0:
                ...
                # 1/(x^0.75) global
                ran = random_doubles4[j]
                x = <int> ((long_nn)*(ran**1.333333333333333333))
        ...
        # if the local recruitment-rate (recruited conversion of A (0) to U (1)) is chosen         
           ...
           # a position of a recruiting nucleosome is chosen
            ...
            if pos_conv == 26 and nuc_conv == 1: #   elif pos_conv == 93
           ...
           if pos_conv == 36 and nuc_conv == 1: #   elif pos_conv == 93
           #if pos_conv==103 and nuc_conv==1:
               # the state of the nucleosome is changed to U
                   mt_region[pos_conv]=2
    ...
    return Max_blue, Max_red, list(states), red
